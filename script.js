...
/* ==========================================================================
   4. MOTEUR HYBRIDE (SUPABASE + NEWSAPI + CACHE)
   ========================================================================== */
async function fetchHybridNews(category = 'top', querySearch = '') {
    const status = document.getElementById('status-line');
    if (status) status.textContent = "CHARGEMENT...";

    try {console.log("Tentative de chargement...");
        // --- 1. SUPABASE ---
...
function renderUI(myArticles, worldNews = []) {
    // On mélange les deux sources : Tes articles d'abord, puis l'API
    const all = [...myArticles, ...worldNews]; 
    
    if (all.length === 0) {
        console.warn("Aucun article à afficher");
        return;
    }
    ...
    if (hero && all[0]) {
        const h = all[0];
        // On gère les deux formats (Supabase utilise 'titre', NewsAPI utilise 'title')
        const displayTitle = h.titre || h.title;
        const displayDesc = h.description || "";
        ...
        hero.innerHTML = `
            <div class="main-article" onclick="captureAction('${displayTitle.replace(/'/g, "\\'")}', '${h.category || 'Monde'}', '${displayLink}')" style="cursor:pointer;">
                ...
                <div class="hero-content">
                    <div class="hero-text">
                        <p style="font-size:1.25rem; color:#333; margin-bottom:15px;">${displayDesc.substring(0, 300)}...</p>
                        <span style="font-weight:bold; color:var(--primary-red); text-transform:uppercase; font-size:0.8rem;">Lire la suite →</span>
                    </div>
                    <div class="hero-image">
                        <img src="${displayImg}" alt="Focus">
                    </div>
                </div>
            </div>`;
    }

    // --- 2. NEWS GRID (Articles 2 à 5) ---
